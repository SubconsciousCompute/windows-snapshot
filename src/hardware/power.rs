...
/// The `Win32_VoltageProbe` WMI class represents the properties of a voltage sensor 
...
pub struct Win32_VoltageProbe {
    /// Accuracy of the sensor for the measured property. The accuracy value is recorded as plus or 
    /// minus hundredths of a percent. Accuracy, along with resolution and tolerance, is used to 
    /// calculate the actual value of the measured physical property. The accuracy may vary and 
    /// depends on whether or not the device is linear in its dynamic range.
    pub Accuracy: Option<i32>,
    /// Availability and status of the device.
    /// 
    /// - `Other` (1)
    ...
    /// Last error code reported by the logical device.
    pub LastErrorCode: Option<u32>,
    /// Sensor threshold values specify the ranges (minimum and maximum values) to determine if 
    /// the sensor is operating under normal, noncritical, critical, or fatal conditions. If 
    /// `CurrentReading` is between `LowerThresholdCritical` and `LowerThresholdFatal`, the current 
    /// state is critical.
    pub LowerThresholdCritical: Option<i32>,
    /// Sensor threshold values specify the ranges (minimum and maximum values) to determine if 
    /// the sensor is operating under normal, noncritical, critical, or fatal conditions. If 
    /// `CurrentReading` is below `LowerThresholdFatal`, the current state is fatal.
    pub LowerThresholdFatal: Option<i32>,
    /// Sensor threshold values specify the ranges (minimum and maximum values) to determine if 
    /// the sensor is operating under normal, noncritical, critical, or fatal conditions. If 
    /// `CurrentReading` is between `LowerThresholdNonCritical` and `UpperThresholdNonCritical`, the 
    /// sensor is reporting a normal value. If `CurrentReading` is between `LowerThresholdNonCritical` 
    /// and `LowerThresholdCritical`, the current state is noncritical.
    pub LowerThresholdNonCritical: Option<i32>,
    ...
    /// - `Timed Power On Supported` (7): Timed Power-On Supported. The `SetPowerState` method can be invoked with the PowerState parameter set to 5 (Power Cycle) and Time set to a specific date and time, or interval, for power-on.
    pub PowerManagementCapabilities: Option<Vec<u16>>,
    /// If `TRUE`, the device can be power-managed, which means that it can be put into suspend mode, 
    /// and so on. The property does not indicate that power management features are currently enabled,
    /// but it does indicate that the logical device is capable of power management.
    pub PowerManagementSupported: Option<bool>,
    /// Ability of the sensor to resolve differences in the measured property. This value may vary and 
    /// depends on whether the device is linear in its dynamic range.
    pub Resolution: Option<u32>,
    /// Current status of the object. Various operational and nonoperational statuses can be defined. 
    /// Operational statuses include: "OK", "Degraded", and "Pred Fail" (an element, such as a 
    /// SMART-enabled hard disk drive, may be functioning properly but predicting a failure in the 
    ...
    /// Name of the scoping system.
    pub SystemName: Option<String>,
    /// Tolerance of the sensor for the measured property. Tolerance, along with resolution and accuracy, 
    /// is used to calculate the actual value of the measured physical property. Tolerance may vary, and 
    /// depends on whether the device is linear in its dynamic range.
    pub Tolerance: Option<i32>,
    /// Sensor threshold values specify the ranges (minimum and maximum values) to determine whether the 
    /// sensor is operating under normal, noncritical, critical, or fatal conditions. If `CurrentReading` is 
    /// between `UpperThresholdCritical` and `UpperThresholdFatal`, the current state is critical.
    pub UpperThresholdCritical: Option<i32>,
    /// Sensor threshold values specify the ranges (minimum and maximum values) to determine whether the sensor 
    /// is operating under normal, noncritical, critical, or fatal conditions. If `CurrentReading` is above 
    /// `UpperThresholdFatal`, the current state is fatal.
    pub UpperThresholdFatal: Option<i32>,
    /// Sensor threshold values specify the ranges (minimum and maximum values) to determine whether the sensor 
    /// is operating under normal, noncritical, critical, or fatal conditions. If `CurrentReading` is between 
    /// `LowerThresholdNonCriticaland` `UpperThresholdNonCritical`, the sensor is reporting a normal value. If 
    /// `CurrentReading` is between `UpperThresholdNonCritical` and `UpperThresholdCritical`, the current state is 
    /// noncritical.
    pub UpperThresholdNonCritical: Option<i32>,
}